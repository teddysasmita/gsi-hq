...
EOS;
	...
	join detailstockexits d on d.id = c.id
	where d.iditem = :p_d_iditem and c.idwarehouse like :p_c_idwh and d.serialnum <> 'Belum Diterima' and
		c.idatetime >= :p_startdatetime and c.idatetime <= :p_idatetime
	group by c.regnum
	order by idatetime
EOS;
				$iditemparam = $_GET['iditem'];
				...
				$startamount = Yii::app()->db->createCommand()
					...
					->where('a.iditem = :p_iditem and a.idwarehouse like :p_idwh and b.idinventorytaking = :p_idinventorytaking',
						array(':p_idinventorytaking'=>$idinventorytaking,
							':p_iditem'=>$iditemparam, ':p_idwh'=>$idwh
						))
						->queryScalar();
