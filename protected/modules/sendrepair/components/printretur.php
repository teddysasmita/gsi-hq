...
class MYPDF extends TCPDF {
	...
	public function LoadData($data, array $detaildata) {
		...
		$this->headernames = array('No', 'Nama Barang', 'Nomor Seri','Gudang');
		$this->headerwidths = array(10, 90, 60, 30);
	}
	...
	public function ColoredTable() {
		...
		*/
		for ($i=0; $i<count($this->detaildata); $i++) {
			...
			$this->Cell($this->headerwidths[3], $ih, lookup::WarehouseNameFromWarehouseID($row['idwarehouse']), 
				'LR', 1, 'L', $fill);
		}
	...
	public function master()
	{
		...
		$this->SetFont('Courier', 'B');
		$this->setXY(91, 10);
		$this->Cell(100, 10, 'KIRIM PERBAIKAN', 'LTR', 1, 'C');
		$this->SetFontSize(10);
		$this->setXY(91, 20);
		...
		$this->SetFont('Courier', 'B');
		//$this->setXY(100, 27);
		$this->Cell(25, 5, 'No Kirim', 'LTR', 0, 'C');
		$this->Cell(15, 5, $this->data->regnum, 'LTR', 1, 'C');
		
		$this->SetFont('Courier', 'B');
		$this->Cell(50, 5, 'Nama Service Center', 'LTRB', 0,'C');
		$this->Cell(140, 5, lookup::CompanyNameFromServiceCenterID($this->data->idservicecenter), 'LTRB', 1, 'C');
		//$this->Cell(190, 15, $this->data->remark, 'LTRB', 1);
...
function execute($model, $detailmodel) {
	...
	$pdf->Output('KIRIMPERBAIKAN-'.$model->regnum.'.pdf', 'D');
}
//============================================================+
// END OF FILE                                                
//============================================================+
