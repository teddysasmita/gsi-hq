...
class MYPDF extends TCPDF {
	...
	public function LoadData2(array $detaildata2) {
		...
		$this->headernames2 = array('No', 'Nama Voucher', 'Nilai');
		$this->headerwidths2 = array(10, 150, 35);
	}
	...
	public function ColoredTable() {
		...
		*/
		...
		$this->Cell(165, 6, 'Total', 'LRTB', 0, 'R');
	...
	public function ColoredTable2() {
		...
		*/
		for ($i=0; $i<count($this->detaildata2); $i++) {
			...
			$this->SetFontSize(10);
			$ih = $this->getStringHeight($this->headerwidths2[1],$row['vouchername']);
			$this->Cell($this->headerwidths2[0], $ih, $counter, 'LR', 0, 'C', $fill);
			$this->MultiCell($this->headerwidths2[1], 0, $row['vouchername'], 'LR', 'L',
					false, 0);
			// 0, 0, true, 0, false, true, 0, 'T', false);
			$this->Cell($this->headerwidths2[2], $ih, number_format($row['vouchervalue']), 'LR', 0, 'L', $fill);
			$this->ln($ih);
	...
	public function footer()
	{
		...
		$this->Cell(86, 5, lookup::UserNameFromUserID($this->data->userlog), 'LRB', 0, 'C', false,'', 0, false, 'T', 'T');
		$this->setX(173);
